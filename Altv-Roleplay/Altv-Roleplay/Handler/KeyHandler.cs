        ...
        public async Task PressE(IPlayer player)
        {
            lock (player)
            {
                ...
                if (vehicleShopPos != null && !player.IsInVehicle)
                {
                    ...
                    //ACLS
                    if (vehicleShopPos.id == 10 && ServerFactions.GetCharacterFactionId(charId) != 5 && !CharactersInventory.ExistCharacterItem(charId, "Fahrzeugschluessel LSC", "schluessel")) { HUDHandler.SendNotification(player, 3, 5000, $"Du hast hier keinen Zugriff drauf. [ACLS]"); return; }
                    //LSF
                ...
                if (vehicleSellPos != null && player.IsInVehicle)
                {
                    ...
                    //ACLS
                    if (vehicleSellPos.id == 10 && ServerFactions.GetCharacterFactionId(charId) != 5 && !CharactersInventory.ExistCharacterItem(charId, "Fahrzeugschluessel LSC", "schluessel")) { HUDHandler.SendNotification(player, 3, 5000, $"Du hast hier keinen Zugriff drauf. [ACLS]"); return; }
                    //LSF
